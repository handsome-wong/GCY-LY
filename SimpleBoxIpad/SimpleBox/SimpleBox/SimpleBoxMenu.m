...
-(id)initWithOrigin:(CGPoint)origin andDiameter:(CGFloat)diameter andDelegate:(UIViewController *)controller themeColor:(UIColor *)themeColor menuColor:(UIColor *)color{
...
-(void)setUpSomeDatas{
    ...
    for (NSInteger i = 0; i < menuCount; i++) {
        ...
        item.alpha = 0.8;
        item.image = [UIImage imageNamed:[_pictures objectAtIndex:i]];
...
-(void)menuTap:(UITapGestureRecognizer *)tapGes{
    ...
    for (NSInteger i = 0; i<menuCount; i++) {
        if ((tapGes.view.tag-_flag == i+1) && [self.menuDelegate respondsToSelector:@selector(menuDidSelected:)]) {
            [self.menuDelegate menuDidSelected:tapGes.view.tag];
        }
    }
...
//        [verticalLineLayer addAnimation:morph_right forKey:@"bounce_0_1_right"];
        ...
        for (UIView *item in Menus) {
            ...
            item.hidden = NO;
            [UIView animateWithDuration:1.0f delay:0.05*(item.tag-_flag) usingSpringWithDamping:0.4f initialSpringVelocity:0.0 options:UIViewAnimationOptionCurveEaseIn | UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionAllowUserInteraction animations:^{
                
                NSValue *pointValue = [PointsDic objectForKey:[NSString stringWithFormat:@"center%ld",item.tag-_flag]];
        ...
        for (UIView *item in Menus) {
            ...
            [UIView animateWithDuration:0.3f delay:0.05*(item.tag-_flag) options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionAllowUserInteraction animations:^{
